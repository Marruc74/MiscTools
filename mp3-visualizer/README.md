...
2. Choose a **visualizer style**, **color theme**, **resolution** (includes 16:9, 1:1 square, and 9:16 vertical for Reels/Shorts), and **video format** (**WebM** by default, or **MP4** for universal playback — MP4 recording needs Chrome or Edge).
   - *(Optional)* **Clip to a section** — set a **Start** and **End** to render and record only part of the track (great for shorts). Accepts `0:30` or plain seconds `30`; leave blank for the whole track. Trimmed clips download with the range in the filename (e.g. `song_30-60s.webm`). When a **Start** is set, the volume fades in over the first 2 seconds; when an **End** is set, it fades to 0 over the last 2 seconds — for a clean intro/outro. (On a short clip each fade is capped to half its length so they don't overlap.)
3. *(Optional)* Add one or more **background images** and set the **second** each appears (or hit **↔ Spread evenly across track** to space them out evenly over the song/clip automatically). Each image has a **pan** slider to shift it horizontally — useful for fitting a widescreen image into a 9:16 frame without being stuck on the center crop — or an **auto →** toggle that slowly sweeps the image left-to-right across the whole time it's on screen, revealing all of it. When any are set, the visualizer shrinks into a rounded panel composited over the photo, and you can choose its **position** — a bottom bar or any of the four corners.
...
Add one or more images as a full-frame background. The **↔ Spread evenly across track** button (shown once a track is loaded and you have 2+ images) distributes the images evenly over the song — or over the trimmed clip, if a Start/End is set — giving each an equal slice of time (image *i* of *n* starts at *i · duration / n*, the first at 0). Each row in the list has an **"at … s"** field — the second at which that image becomes active — plus two ways to control horizontal framing of a too-wide image:
...
Pan/auto-pan only have a visible effect when the image is wider than the frame can show (e.g. a 16:9 photo in a 9:16 canvas); otherwise there's no horizontal slack to move within.

**Portrait images in a wide frame** are handled differently: rather than zooming in and cropping a tall photo to fill a 16:9 (or other landscape) canvas, the image is **fit to the frame height and centered, with black bars at the sides** so the whole photo stays visible. Vertical (9:16) and square frames keep the normal cover behavior (the image fills the frame). Pan/auto-pan are ignored for a letterboxed portrait image since all of it is already shown. As playback passes each time, the background **crossfades** (1.2 s) from the previous image to the next, so a sequence of images plays as a synced slideshow behind the music. When any image uses **auto →**, the idle canvas loops the timeline so you can preview the motion (and slideshow) before recording.
